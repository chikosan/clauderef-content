...
- **Persistent context and cost visibility:** subagents can keep `memory: project|user|local` across sessions. `/usage` now shows scheduled-task usage, `promptCacheTtl` can keep API/cloud prompt caches warm for an hour, and `/loop` usage is easier to audit.
...
The webinar announcement and blog recommendations are useful ecosystem news, but they are not Claude Code reference behavior; check the linked event or post directly for dates and availability.
...
[Desktop sessions](https://code.claude.com/docs/en/desktop) · [Remote Control](https://code.claude.com/docs/en/remote-control) · [Session resume](https://code.claude.com/docs/en/sessions#resume-a-session)
